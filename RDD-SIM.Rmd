...
subtitle: "A Simulated Data Example"
author: "EDS 241"
output: 
    html_document:
      theme: flatly
...
### Using package {`rdrobust`}

------------------------------------------------------------------------

**RDD Robust Estimation Method (`local polynomial regression`)**2: 

Local polynomial regression is a method used to estimate relationships between variables while giving more weight to observations near a specific point— in this case, the cutoff in a Regression Discontinuity Design (RDD). Instead of fitting an ordinary linear regression, it fits separate curved-regressions on either side of the cutoff using only nearby data points. The method allows for flexibility in capturing non-linear patterns.

This tutorial walks you through using **Regression Discontinuity Design (RDD)** to estimate causal effects when treatment assignment is determined by a cutoff value. We'll explain each step in detail so that even if you're new to RDD, you can follow along!

**Interpreting output**:

*Default estimation options used by the `rdrobust()` function*:

- Bandwidth Optimization (`BW type: mserd`): The Mean Squared Error optimal bandwidth is chosen to balance accuracy & bias.
- Bandwidth Estimate (`h = 0.461`): This is the range around the cutoff (X = 0) where the model uses a subste of the data to estimate the treatment effect.
- Kernel (`Triangular`): Gives more weight to data points closer to the cutoff, meaning observations near (`X=0`)
- Variance Estimation (Nearest Neighbor; `VCE method: NN`): Instead of assuming equal variance across all observations, this method adjusts the uncertainty estimates by considering how variability changes near the cutoff.
...
```{r}
# Run rdrobust with sharp cutoff at X = 0
rdd_model <- rdrobust(Y, X, c = 0)
...
### Make figure for lecture slides
